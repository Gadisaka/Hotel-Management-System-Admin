...
const Sidebar: React.FC = () => {
  ...
  return (
    <div
      className={`h-screen flex relative justify-between gap-1 shadow-md ${
    ...
    >
      <div className="flex flex-col items-start  w-full justify-between">
        <div className="">
          <div className="text-2xl font-bold  w-full  flex  items-center gap-x-4 p-3  text-blue-500">
            ...
            {!isCollapsed && <h1 className="ml-4">D _OTEL.</h1>}
          </div>
        </div>
            ...
            {links.map((link) => (
              <li key={link.path}>
                <NavLink
                  to={link.path}
                  className={({ isActive }) =>
                    `flex hover:bg-blue-100 hover:text-blue-500 gap-x-6 my-2  cursor-pointer rounded-lg pl-2 p-2 ${
                      isActive ? "bg-blue-100 text-blue-600 font-semibold" : ""
                    }`
                  }
                >
                  <span>{link.icon}</span>
                  {!isCollapsed && <h1 className="ml-4">{link.name}</h1>}
                </NavLink>
              </li>
            ))}
          </ul>
      ...
      <div className="rounded-full right-[-30px] absolute w-8 h-8 p-3 mt-3  z-10 flex justify-center items-center  bg-white shadow-md border ">
        <button onClick={toggleSidebar}>
          {!isCollapsed ? (
            ...
            >
              <path
                d="M1.25 1.51624L8.75 9.01624L1.25 16.5162"
                stroke="#5D6679"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
